...
implement main0 () = let
  ...
  val () = println! ("\n=== Content text: compile-time character verification ===")
  ...
  val b = ward_content_text_putc(b, 5, 47) (* '/' — allowed in SAFE_CONTENT_CHAR *)
  ...
  val ct = ward_content_text_done(b)
  val v0 = byte2int0(ward_safe_content_text_get(ct, 0))
  val v5 = byte2int0(ward_safe_content_text_get(ct, 5))
  val () = println! ("content_text[0] = ", v0, " ('i'=105)")
  val () = println! ("content_text[5] = ", v5, " ('/'=47)")
  val () = ward_safe_content_text_free(ct)
  val () = println! ("content text freed")

  (* === Content text: ward_text_to_content === *)
  val () = println! ("\n=== Content text: ward_text_to_content ===")
  val ct2 = ward_text_to_content(attr_class, 5)
  val v0 = byte2int0(ward_safe_content_text_get(ct2, 0))
  val () = println! ("converted safe_text[0] = ", v0, " ('c'=99)")
  val () = ward_safe_content_text_free(ct2)
  val () = println! ("converted content text freed")
  ...
  (* === Arena: create, alloc, get/set, return, destroy === *)
  val () = println! ("\n=== Arena: create, alloc, get/set, return, destroy ===")
  val arena = ward_arena_create(65536)
  val @(tok1, arr1) = ward_arena_alloc<byte>(arena, 1024)
  val () = ward_arr_set<byte>(arr1, 0, int2byte0(42))
  val () = ward_arr_set<byte>(arr1, 1023, int2byte0(99))
  val v0 = byte2int0(ward_arr_get<byte>(arr1, 0))
  val v1 = byte2int0(ward_arr_get<byte>(arr1, 1023))
  val () = println! ("arena arr1[0] = ", v0, ", arr1[1023] = ", v1)
  val () = assertloc(v0 = 42)
  val () = assertloc(v1 = 99)
  val @(tok2, arr2) = ward_arena_alloc<int>(arena, 256)
  val () = ward_arr_set<int>(arr2, 0, 12345)
  val () = ward_arr_set<int>(arr2, 255, 67890)
  val v2 = ward_arr_get<int>(arr2, 0)
  val v3 = ward_arr_get<int>(arr2, 255)
  val () = println! ("arena arr2[0] = ", v2, ", arr2[255] = ", v3)
  val () = assertloc(v2 = 12345)
  val () = assertloc(v3 = 67890)
  (* Exercise freeze/thaw on arena arrays — proves full ward_arr protocol *)
  val @(frozen, borrow) = ward_arr_freeze<byte>(arr1)
  val v4 = byte2int0(ward_arr_read<byte>(borrow, 0))
  val () = assertloc(v4 = 42)
  val borrow2 = ward_arr_dup<byte>(frozen, borrow)
  val () = ward_arr_drop<byte>(frozen, borrow)
  val () = ward_arr_drop<byte>(frozen, borrow2)
  val arr1 = ward_arr_thaw<byte>(frozen)
  val () = println! ("arena freeze/thaw OK")
